/* =====================================================================
   QLog Pro — Teacher Return Clearance & Archive v1.0
   ...
   - Opens the browser print dialog after successful teacher returns.
...
function safeName(v){return txt(v).replace(/[^A-Za-z0-9._-]+/g,'_').replace(/^_+|_+$/g,'').slice(0,70)||'Teacher';}
...
function makeDocXml(rec){
  ...
  body+=table([
    ['Teacher / Borrower',rec.borrowerName],['ID Number',rec.borrowerId],['Category',rec.borrowerCategory||'Teaching Personnel'],['Office / Facility',rec.facilityName||''],['Issued',rec.issuedAt],['Accountability Status',status]
  ],[2700,6500]);
  body+=para('',{after:70});
  ...
  if(rec.remaining&&rec.remaining.total>0) statement+='This return clearance applies only to the listed item(s). Remaining active accountabilities: '+rec.remaining.books+' book(s) and '+rec.remaining.equipment+' equipment item(s).';
  else statement+='No active book or equipment accountability remains in QLog Pro for this borrower as of issuance.';
  body+=para(statement,{size:20,after:180});
  body+=table([['Received / Certified By','Teacher / Borrower'],[rec.receivedBy||'____________________________',rec.borrowerName||'____________________________'],[rec.receivedByDesignation||'Unit In-Charge','Signature / Acknowledgment']], [4600,4600]);
...
function buildRecord(opts){
  ...
  var status=remaining.total===0?'FULLY CLEARED':'RETURN CLEARED — OTHER ACCOUNTABILITIES REMAIN';
...
async function issue(opts){
  opts=opts||{};var p=opts.person||getPerson(opts.borrowerId)||{};var cat=opts.borrowerCategory||p.category;
  if(!isTeacherCategory(cat)) return {issued:false,reason:'NOT_TEACHER'};
  var rec=buildRecord(opts);var blob=await generateDocx(rec);rec.filename='QLog_Clearance_'+safeName(rec.borrowerName)+'_'+rec.id+'.docx';rec.docxBlob=blob;await put(rec);
  if(global.toast) global.toast('📁 Teacher clearance archived: '+rec.id,'green');
...
async function archiveHtml(scope){var rows=(await all()).filter(function(r){return recordVisible(r,scope);});if(!rows.length)return '<tr><td colspan="8" style="text-align:center;color:#64748b;padding:20px;">No teacher clearances archived for this scope.</td></tr>';return rows.map(function(r){var items=(r.items||[]).map(function(x){return x.name||x.title||x.id||x.isbn;}).join(', ');return '<tr><td>'+escHtml(r.id)+'</td><td>'+escHtml(r.borrowerName)+'</td><td>'+escHtml(r.borrowerId)+'</td><td>'+escHtml(r.module)+'</td><td>'+escHtml(r.facilityName)+'</td><td>'+escHtml(items)+'</td><td>'+escHtml(r.issuedAt)+'</td><td><button onclick="QLogClearance.download(\''+escHtml(r.id)+'\')" style="padding:5px 9px;font-size:12px;">DOCX</button> <button onclick="QLogClearance.print(\''+escHtml(r.id)+'\')" style="padding:5px 9px;font-size:12px;background:#0f172a;">Print</button></td></tr>';}).join('');}
async function openArchive(scope){
  var ov=document.getElementById('qlogClearanceArchiveModal');if(!ov){ov=document.createElement('div');ov.id='qlogClearanceArchiveModal';ov.className='modal-overlay';ov.innerHTML='<div class="modal-content" style="max-width:1180px;"><div class="modal-header"><div><h3 style="margin:0;">📁 Teacher Clearance Archive</h3><div style="font-size:12px;color:#64748b;margin-top:3px;">Archived DOCX clearances remain available offline for future download or printing.</div></div><button class="modal-close" onclick="document.getElementById(\'qlogClearanceArchiveModal\').style.display=\'none\'">Close</button></div><div class="full-table"><table><thead><tr><th>Clearance No.</th><th>Teacher</th><th>ID</th><th>Module</th><th>Facility</th><th>Returned Item(s)</th><th>Issued</th><th>Actions</th></tr></thead><tbody id="qlogClearanceArchiveTbl"></tbody></table></div></div>';document.body.appendChild(ov);}ov.style.display='flex';var tb=document.getElementById('qlogClearanceArchiveTbl');tb.innerHTML='<tr><td colspan="8">Loading clearance archive...</td></tr>';try{tb.innerHTML=await archiveHtml(scope||'');}catch(e){tb.innerHTML='<tr><td colspan="8" style="color:#dc2626;">Unable to load archive: '+escHtml(e.message||e)+'</td></tr>';}}
